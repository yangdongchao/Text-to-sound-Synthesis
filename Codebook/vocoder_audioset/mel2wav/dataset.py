...
def files_to_list(filename, data_root): # 根据txt中的文件名+data_root读取已经保存好的.npy文件
    """
    Takes a text file of filenames and makes a list of filenames
    """
    with open(filename, encoding="utf-8") as f:
        files = f.readlines()
    #f'{f.rstrip()}_audio.npy'
    files = [data_root / f'{f.rstrip()}_audio.npy' for f in files]
    # print(files[0])
    # assert 1==2
    #files = [data_root / f"{f.rstrip().split('/')[0]}" / f"{f.rstrip().split('/')[1]}_audio.npy" for f in files]
    return files
...
class AudioDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Take segment
        if audio.size(0) >= self.segment_length: # 目前每段音频只取 segment_length
