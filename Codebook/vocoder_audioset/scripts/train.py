...
def parse_args():
    ...
    parser.add_argument("--load_path", default='/apdcephfs/share_1316500/donchaoyang/code3/SpecVQGAN/vocoder_audioset/logs/audioset')

    parser.add_argument("--n_mel_channels", type=int, default=80)
    parser.add_argument("--ngf", type=int, default=32) # 32
    parser.add_argument("--n_residual_layers", type=int, default=3) # 3

    parser.add_argument("--ndf", type=int, default=16) # 16
    parser.add_argument("--num_D", type=int, default=3) # 3
    parser.add_argument("--n_layers_D", type=int, default=4) #
    parser.add_argument("--downsamp_factor", type=int, default=4)
    parser.add_argument("--lambda_feat", type=float, default=10)
    parser.add_argument("--cond_disc", action="store_true")

    parser.add_argument("--data_path", default='/apdcephfs/share_1316500/donchaoyang/data/audioset/features', type=Path)
...
def main():
    ...
    # Create optimizers #
    #####################
    optG = torch.optim.Adam(netG.parameters(), lr=5e-5, betas=(0.5, 0.9))
    optD = torch.optim.Adam(netD.parameters(), lr=5e-5, betas=(0.5, 0.9))
    ...
    train_set = AudioDataset(
        Path(args.data_path), Path(args.splits_path) / 'audioset_train_vocoder_part.txt', args.seq_len, sampling_rate=22050
    )
    test_set = AudioDataset(
        Path(args.data_path),
        Path(args.splits_path) / 'caps_valid_vocoder.txt',
        22050 * 10,
        sampling_rate=22050,
        augment=False,
    )
    ...
    for i, x_t in enumerate(test_loader):
        x_t = x_t.cuda()
        s_t = wav2mel(x_t) # transfer wav to mel
    ...
    for epoch in range(1, args.epochs + 1):
        for iterno, x_t in enumerate(train_loader):
            ...
            # 256 = hoplen
            # print('x_t ',x_t.shape)
            trim_len = x_t.shape[-1] // 256 # 计算生成mel谱的长度
            # print('trim_len ',trim_len)
            s_t = wav2mel(x_t.squeeze(1), trim_len) # [16, 80, 32]
            # print('s_t ',s_t.shape)
            # assert 1==2 
            x_pred_t = netG(s_t.cuda())
            with torch.no_grad():
                s_pred_t = wav2mel(x_pred_t.squeeze(1).detach().cpu(), trim_len)
                s_error = F.l1_loss(s_t, s_pred_t).item() # only the time domain loss?
            ...
            if steps % args.log_interval == 0:
                ...
                start = time.time()
        end_epoch = time.time()
        print('epoch, time(mins) ', epoch, (end_epoch-start_train_ticks)/60)
